...
from ..tooluse.memory_tool import *
...
class RayPPOTrainer:
    ...
    def get_second_rollout_batch(self, og_output_gen_batch, original_full_batch, append=False):
        memories = original_full_batch.non_tensor_batch["memory"]
        prompts = original_full_batch.non_tensor_batch["prompt"]
        img_paths = original_full_batch.non_tensor_batch["img_paths"]
        ...
        for idx, result in enumerate(parsed_results):
            ...
            if not result["status"]:
                if result["error_code"] == "NOTOOL":
                    num_direct += 1
                    original_full_batch.non_tensor_batch["notool"][idx] = 1
                else:
            ...
            res = self.actor_rollout_wg.exec_code(result, memories[idx])
            successful, captured_output = res[0]
            ...
            if successful:
                if self.captured_output is not None:
                    successful = isinstance(self.captured_output, list) and len(self.captured_output) > 0
                else:
                    successful = False
            ...
            if successful:
                ...
                all_imgs = []
                for img_path in img_paths[idx]:
                    img_full_path = os.path.join(self.config.data.data_path, img_path)
                    with Image.open(img_full_path) as original_image:
                        original_image = original_image.convert("RGB")
                        all_imgs.append(original_image)

                for content in self.captured_output:
                    if isinstance(content, dict) and 'img' in content:
                        all_imgs.append(content['img'])
                    else:
                        print("Warning: unexpected captured_output type:", type(content))

                print("img_length:", len(all_imgs))
                
                messages = self.val_dataloader.dataset.my_build_message(prompts[idx], trim_to_action_end, self.captured_output)
                ...
                for i, image in enumerate(all_imgs):
                    if image is None:
                        if i == 0:
                            ...
                            image = all_imgs[i-1]
                    img_proc = self.val_dataloader.dataset.process_image(image)
                ...
                if len(raw_prompt_ids) > max_prompt_length:
                    if self.val_dataloader.dataset.truncation == "left":
                        raw_prompt_ids = raw_prompt_ids[-max_prompt_length :]
                    elif self.val_dataloader.dataset.truncation == "right":
                        raw_prompt_ids = raw_prompt_ids[: max_prompt_length]
                    elif self.val_dataloader.dataset.truncation == "error":
        ...
        if append:
            ...
            current_len = len(second_rollout_datas) + len(img_paths)
        ...
        print(second_rollout_batch.batch.keys())
        print("NON TENSOR KEYS")
        print(original_full_batch.non_tensor_batch.keys())
        print(second_rollout_batch.non_tensor_batch.keys())
    ...
    def _validate(self) -> Dict[str, Any]:
        ...
        for batch_dict in self.val_dataloader:
            ...
            test_batch, batch_tool_stats = self.get_second_rollout_batch(test_output_gen_batch, test_batch)
