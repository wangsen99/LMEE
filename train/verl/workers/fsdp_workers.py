...
class FSDPWorker(Worker):
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def exec_code(self, result, memories, top_k=5):
        code = result["content"]
        successful = True
        self.captured_output = None

        safe_context = self.get_tool_context()
        safe_context["memory"] = memories
        safe_globals = {"__builtins__": {}}

        try:
            exec(code, safe_globals, safe_context)
        except BaseException as e:
            successful = False
            print(f"[exec_code ERROR] {e}")

        if isinstance(self.captured_output, list) and len(self.captured_output) > top_k:
            self.captured_output = self.captured_output[:top_k]
        ...
        def is_valid_output(obj):
            ...
            for i, item in enumerate(obj):
                if not isinstance(item, dict):
                    print(f"[exec_code WARNING] Item {i} is not dict:", type(item))
                    return False
                if "img" not in item or "objects" not in item:
                    print(f"[exec_code WARNING] Item {i} missing keys:", item.keys())
                    return False
                # 检查 img 类型
                if not hasattr(item["img"], "size") or not hasattr(item["img"], "mode"):
                    print(f"[exec_code WARNING] Item {i} has invalid img type:", type(item["img"]))
                    return False
                # 检查 objects 类型
                if not isinstance(item["objects"], str):
                    print(f"[exec_code WARNING] Item {i} has invalid objects type:", type(item["objects"]))
                    return False
            return True

        if not is_valid_output(self.captured_output):
            print("[exec_code WARNING] Invalid captured_output:", self.captured_output)
            successful = False
            self.captured_output = None

        return successful, self.captured_output
