...
class LLMBatchMEMFunctionRewardManager(FunctionRewardManager):
    ...
    def compute_reward(self, data: DataProto) -> Tuple[torch.Tensor, Dict[str, List[float]]]:
        response_str, ground_truth = [], []
        labels, qa_category, qa_answer = [], [], []
        # labels, found_objects = [], []
        notool = []
        response_ids = data.batch["responses"]
        response_length = data.batch["response_mask"].sum(dim=-1)
        for i in range(len(data)):
            ...
            if i % 50 == 0:
                print(response_str[i])

            ground_truth.append(data.non_tensor_batch["ground_truth"][i])
            labels.append(data.non_tensor_batch["label"][i])
            notool.append(data.non_tensor_batch["notool"][i])
            qa_category.append(data.non_tensor_batch["qa_category"][i])
            qa_answer.append(data.non_tensor_batch["qa_answer"][i])

        scores = self.reward_fn(response_str, ground_truth, labels, qa_category, qa_answer, notool)
        reward_tensor = torch.zeros_like(data.batch["responses"], dtype=torch.float32)
        reward_metrics = defaultdict(list)
        for i, score in enumerate(scores):
            reward_tensor[i, response_length[i] - 1] = score["overall"]
